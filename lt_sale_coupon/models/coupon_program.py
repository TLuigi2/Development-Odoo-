...
class CouponProgram(models.Model):
    ...
    state_coupon_progrm =  fields.Selection([('active', 'Activo'),('inactive','Inactivo')], copy=False, default='active', string="Estado")

    # The api.depends is handled in `def modified` of `lt_sale_coupon/models/sale_order.py`
    ...
    def _filter_programs_on_products(self, order):
        ...
        for program in self:
            ...
            if program.promo_applicability == 'on_current_order' and program.reward_type == 'product':  #and program._get_valid_products(program.reward_product_id): mode negbla
                #print('#'*30, f'PRINTACOMEN ENTRA  A CONDICION')
                pass
                #ordered_rule_products_qty -= program.reward_product_quantity negbla
            if ordered_rule_products_qty >= program.rule_min_quantity:
                valid_program_ids.append(program.id)
                encontrado =  self.browse(valid_program_ids)
    ...
    def _filter_programs_from_common_rules(self, order, next_order=False):
        ...
        if programs_curr_order:
            ...
            #Negbla se comenta la siguiente linea por que valida que el producto gratis se encuentre en la linea 
