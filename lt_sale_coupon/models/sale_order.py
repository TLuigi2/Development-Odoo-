...
class SaleOrder(models.Model):
    ...
    def _get_reward_values_product(self, program):
        pass #!Modificacion de codigo interno
        price_unit = 0
    ...
    def _get_reward_values_percentage_amount(self, program):
        ...
        if program.discount_apply_on == 'cheapest_product_neg':
        ...
        elif program.discount_apply_on in ['specific_products_neg', 'on_order']:
            if program.discount_apply_on == 'specific_products_neg':
    ...
    def _send_reward_coupon_mail(self):
        template = self.env.ref('lt_sale_coupon.mail_template_lt_sale_coupon', raise_if_not_found=False)
        if template:
            for order in self:
